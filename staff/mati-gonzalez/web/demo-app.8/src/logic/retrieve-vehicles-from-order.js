...
function retrieveVehiclesFromOrderDetails(token, orderId) {
    validateToken(token)
    validateString(orderId)

    return fetch('https://b00tc4mp.heorkuapp.com/api/v2/users', {
        headers: {
                            ...
                            return fetch(`https://b00tc4mp.herokuapp.com/api/hotwheels/vehicles/${item.id}`)
                                .then(res => {
                                    ...
                                    if (status === 200) {
                                        return res.json()
                                            .then(vehicles => {
                                                vehicles.qty = item.qty
                                                vehicles.total = vehicle.price * vehicle.qty
