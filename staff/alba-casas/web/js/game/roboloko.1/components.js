...
class Roboloko extends Shape {
  constructor(color, width, height) {
    ...
    setInterval(() => {
      if (pupil.x <= 0) xDir = "right";
      else if (pupil.x >= xLimit) xDir = "left";

      if (xDir === "right") pupil.moveRight(Math.random());
      else if (xDir === "left") pupil.moveLeft(Math.random());

      if (pupil.y <= 0) yDir = "down";
      else if (pupil.y >= yLimit) yDir = "up";

      if (yDir === "down") pupil.moveDown(Math.random());
      else if (yDir === "up") pupil.moveUp(Math.random());
    }, 100);
  ...
  toggleTooth() {
    ...
    if (style.backgroundColor === "white") style.backgroundColor = "black";
