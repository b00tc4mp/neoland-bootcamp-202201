...
function findActions(token, query, requiredTime, requiredBudget) {
    ...
    return fetch(`http://localhost:8080/api/actions/?${query !== null ? `query=${query}` : ''}&${requiredTime !== null ? `requiredTime=${requiredTime}` : ''}&${requiredBudget !== null ? `requiredBudget=${requiredBudget}` : ''}`, {
        headers: {
